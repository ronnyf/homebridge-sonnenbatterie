...
export class SonnenBatterieConsumptionAccessory<P extends PlatformAccessory>
  implements UpdatableAccessory
{
  ...
  constructor(
    ...
    const consumptionService =
      accessory.getService("SB-Cons-ID") ||
    ...
    consumptionService.addOptionalCharacteristic(
    ...
    this.platform.log.debug(
      "did bind characteristics to service: ",
      consumptionService.UUID,
    );
  ...
  async hasLowBattery(): Promise<CharacteristicValue> {
    const value =
      this.platform.sonnenAPI.batteryStatus.USOC <=
      this.platform.sonnenAPI.batteryStatus.BackupBuffer;
    return value;
  }
  ...
  updateAccessory(batteryStatus: BatteryStatus, _: InverterStatus) {
    ...
    this.platform.log.info("consumption: ${batteryStatus.Consumption_W}");
